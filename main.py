...
def train(param):
    ...
    D_OPT_PATH = './tmp/' + NAME + '_' + \
                 SAMPLINE_METHOD + '_' + STATE_METHOD + '/'
    ...
    while episode < EPISODES:
        ...
        while step < STEPS:
            ...
            if op_name in O2:
                ...
                model_cluster2.store_transition(state_emb2, acts2, r_c2, state_emb2_, action_list)
            model_cluster1.store_transition(state_emb, acts1, r_c1, state_emb_, action_list)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not os.path.exists('./log/'):
            os.mkdir('./log/')
        if not os.path.exists('./log/nni/'):
            os.mkdir('./log/nni/')
        if not os.path.exists('./log/nni/' + trail_id):
            os.mkdir('./log/nni/' + trail_id)
        if not os.path.exists('./log/nni/' + trail_id + '/' +
                              params['name']):
            os.mkdir('./log/nni/' + trail_id + '/' + params['name'])
        log_file = './log/nni/' + trail_id + '/' + params['name'] + '/' + start_time + '_' + params[
            'state_method'] + '.log'
